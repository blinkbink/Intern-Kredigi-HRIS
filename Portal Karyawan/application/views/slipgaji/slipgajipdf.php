...
<?php
$perusahaan=$this->karyawan_model->get_karir($karyawanID);
//var_dump($perusahaan);
if(!empty($perusahaan[0]['logo_perusahaan'])){ ?>
  <!-- <img src=<?=base_url($perusahaan[0]['logo_perusahaan'])?> alt="" id="gambar" width="30" height="50"> -->
  <img src="http://dev.kredigi.co.id/~yusuf/<?=$perusahaan[0]['logo_perusahaan']?>" alt="" id="gambar" width="30" height="50">
<?php } else { ?>
...
$totalIncome = 0;
$pendapatan = 0;
$karir= $this->slipgaji_model->get_periodegaji_detail($masterGajiID, $periodeAkhirDate, $karyawanID);
//var_dump($karir);
...
<?php
  ...
  $itemGaji = $this->slipgaji_model->getItemGaji($masterGajiID);
  //$totalIncome = 0;
  //$pendapatan = 0;
...
//Menghitung nilai pendapatan 
	...
	//Menghitung unit pendapatan tergantung kehadiran/lembur
        $unitQ = $this->slipgaji_model->getUnit($value['periode_gaji_id'], $key['data_ID']);
	...
	if(!empty($unitQ)){
	  ...
	  $absensi = $this->Kehadiran_model->getDataAbsensiRekap($karir[0]['kode_absensi'],$dateAwal, $dateAkhir);
...
?>
                      <tr>
                        ...
                        <?php
                         if(!empty($rate)){
                                 echo "Rp ".number_format($rate,0,'','.');
                        } ?>
                        </td>
                        <td class='text-right'>
                              <?php echo "Rp ".number_format($income,0,'','.');?>
                        </td>
                      </tr>
<?php
...
?>
  ...
  ?>
                        <tr>
                          ...
                          <td></td>
                          <td></td>
                          <td class='text-right'>Rp <?=number_format($potong,0,'','.')?></td>
                        </tr>
  <?php
...
?>
                    ...
                    <thead>
                      <tr class="success">
                        ...
                        <td></td>
                        <td></td>
                        <td class='text-right'>Rp <?=number_format($thp,0,'','.')?></td>
                      </tr>
                    </thead>
                  </table>
                </div>
