            ...
            <div class="box-body">
              <table class="table table-hover table-striped" id="datatable">
                ...
                <?php foreach ($karyawan as $key => $value) { 
                  ...
                  //menghitung lamanya kerja sejak bergabung
                  if(empty($value->mulai_kerja)){//jika tidak ada value mulai_kerja
                    $mulaiKerja = '';
                  } else {
                    $mulaiKerja = durasi(strtotime($value->mulai_kerja));
                  }
                  
                  $portal = $this->Karyawan_model->get_user($value->karyawan_ID);
                  //var_dump($portal);
                  if(!empty($portal['active'])){
                      if($portal['active'] === '1'){
                          $statusPortal = "Aktif";
                      } else {
                          $statusPortal = "Tidak aktif";
                      }
                  } else {
                      $statusPortal = "Tidak aktif";
                ...
                <tr>
                  ...
                  <td><?=$jobtitle?></td>
                  <td><?=$mulaiKerja?></td>
                  <td><?=$value->kode_absensi?></td>
                  <td><?=$value->hp_karyawan?></td>
                  <td><?=$value->email_karyawan?></td>
                  <td><?php echo anchor('karyawan/changeportal/'.$value->karyawan_ID.'/'.$statusPortal, $statusPortal); ?></td>
                  <td>
                    <div class="btn-group">
