...
<section class="content">
    ...
    <div class="box">
        <div class="box-header with-border">
            <h3 class="box-title">Tugas Luas Kantor</h3>
        ...
        <div class="box-body">
            <button onclick="location.href='<?php echo base_url(). "index.php/izin/add_tugas"?>'" class="btn display-block font-normal btn-success">+ Pengajuan Tugas Luar Kantor</button>
            <br><br>
            ...
            <table class="table table-hover table-striped" id="datatable">
                ...
                <tr class="success">
                    ...
                    <th>Tanggal Tugas Luar Kantor</th>
                ...
                if($tugas != null)
                {
                    ...
                    <tr>
                        ...
                        <td>
                            <?php
                            $convert = strtotime($value['tanggal_penetapan']);
                            echo $hari[date("w", $convert)].", ".date("j", $convert)." ".$bulan[date("n", $convert)]." ".date("Y", $convert);
                            ?>
                        </td>
                        <td>
                            <?php
                            $convert = strtotime($value['tanggal_tugas']);
                            echo $hari[date("w", $convert)].", ".date("j", $convert)." ".$bulan[date("n", $convert)]." ".date("Y", $convert);
                            ?>
                        </td>
                        ...
                        <td>
                            <div class="btn-group">
                                <a href="<?=site_url('izin/detail_tugas/'.$value['id_tugas'])?>" type="button" class="btn btn-sm btn-info" >
                                    <i class="fa fa-file" data-toggle="tooltip" data-placement="bottom" role="presentation" title="Detail"></i>
                                </a>
                                <button type="button" class="btn btn-sm btn-warning hapus-pengumuman" data-id="<?=$value['id_tugas']?>" jumlah_hari="<?=$value['jumlah_hari']?>" tanggal_tugas="<?=$value['tanggal_tugas']?>" data-karir="0" karyawan_ID="<?=$value['karyawan_ID']?>">
            ...
            <div class="modal fade" tabindex="-1" role="dialog" id="myModal">
                <div class="modal-dialog modal-sm" role="document">
                    <div class="modal-content">
                        <form action="<?php echo base_url("index.php/izin/ubah_tugas") ?>" role="form" class="form-horizontal" method="post">
                            <div class="modal-header">
                                <h4 class="modal-title" id="myModalLabel">Persetujuan Izin Tugas</h4>
                            </div>
                            <div class="modal-body">
                                <table class="table">
                                    <tr>
                                        <td>Ubah Persetujuan
                                            <input type="hidden" name="id_tugas" id="id_data" value="">
                                            <input type="hidden" name="karyawan_ID" id="jjudul" value="">
                                            <input type="hidden" name="jumlah_hari" id="jml" value="">
                                            <input type="hidden" name="tanggal_tugas" id="tgl" value="">
...
<script>
    $(function () {
        ...
        $(".hapus-pengumuman").click(function(){
            ...
            var divTgl = $(this).attr('tanggal_tugas');
            ...
            if(divKarir == '0'){
                ...
                $('#myModal .modal-header #myModalLabel').text('Persetujuan Izin Tugas');
