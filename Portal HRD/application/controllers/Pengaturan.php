...
class Pengaturan extends MY_Controller {
  ...
  public function __construct(){
    parent::__construct();    
    $this->load->model(array('Data_model', 'Slip_gaji_model', 'Pola_kerja_model'));
...
//input data pola kerja
public function pola_kerja($param='',$action='',$id='')
  {
      ...
      if($action=='edit'){
        ...
        if($count_data>0){
          ...
          $this->load->view('pengaturan/form_gaji',$data);
           ...
           redirect('pengaturan/pola_kerja/list');
        }
	...
	echo $id;
	$data['pola_kerja'] = $this->Pola_kerja_model->getListData($id);
	$data['pola_kerja_hari'] = $this->Pola_kerja_model->getPolaKerja($id);
	$this->load->view('pengaturan/pola_kerja_detail', $data);
      }
      elseif($action=='tambah' || $action=='update'){
        //$rules = $this->Pola_kerja_model->rules;
        //$this->form_validation->set_rules($rules);
        ...
        //if ($this->form_validation->run() == TRUE) {
          ...
          $dataA=array(
                'perusahaan_ID' => get_user_info('perusahaan_ID'),
                'nama_polker' => $post['pola_kerja'],
                //'total_jam_kerja' => $post['toleransi'],
                'toleransi_telat' => $post['toleransi']
          );

          //print_r($post);

          if($action=='tambah'){
            $getID = $this->Pola_kerja_model->insertPolaKerja($dataA);
	    extract($_POST);
	    ...
	    foreach($hari as $key=>$value){
			...
			'id_polker' => $getID,
                        'hari_ke' => $value,
                        'jenis_hari' => $_POST['status_hari'][$key],
                        'jam_mulai' => $_POST['jam_masuk'][$key],
                        'jam_selesai' => $_POST['jam_pulang'][$key]
                );
		$this->Pola_kerja_model->insertPolaHari($dataB);
            }
            $this->site->activity_log('Menambahkan Pola Kerja','pola_kerja',serialize($dataA),''.$getID.''); 
          }
          elseif($action=='update'){
            $this->Pola_kerja_model->update($data_post, array('master_gaji_ID' => $post['id_data']));
            $getID = $post['id_data'];
            $this->site->activity_log('Merubah Slip Gaji','master_gaji',serialize($data_post),''.$getID.''); 
          }

          if(!empty($getID))
            redirect('pengaturan/pola_kerja/list/edit/'.$getID);
          else{
            redirect('pengaturan/pola_kerja/list');
          }
        //}
        //else
          //redirect('pengaturan/pola_kerja/list/error/gagal-proses');       
      }
      elseif($action=='hapus'){        
        if(!$id==''){     
         //echo $use=$this->Pola_kerja_model->getData($id);//( LIKE '%\"".$id."\"%'");
        
          //if($use==0)  
            $this->Pola_kerja_model->hapusPolaKerja('id_polker = '.$id);
          //else
            //redirect('pengaturan/pola_kerja/list/error/gagal-hapus');
        }
        redirect('pengaturan/pola_kerja/list');
      }
      elseif($action=='error'){
        if(!empty($id)){        
          ...
          $data['error']='<div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-ban"></i> '.$title_error.' </h4>'.$pesan_error.'</div>';
          $data['slip']=$this->Pola_kerja_model->getListData();
          $this->load->view('pengaturan/pola_kerja',$data);
        }
        else{
          redirect('pengaturan/pola_kerja/'.$param);
        }
        ...
        $data['error']='';
        $data['list']=$this->Pola_kerja_model->getListData();
        $this->load->view('pengaturan/pola_kerja',$data);
      }
    }
    elseif($param=='item'){
      ...
      redirect('pengaturan/pola_kerja/list/edit/'.$post['id_slip']);        
    }elseif($param=='form'){
      $this->load->view('pengaturan/pola_kerja_form.php');
    } else {
      redirect('pengaturan/pola_kerja/list');
    }
  }
