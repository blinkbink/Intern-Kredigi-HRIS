...
class Karyawan extends MY_Controller {
  ...
  public function __construct(){
    parent::__construct();    
    $this->load->model(array('Karyawan_model','Data_model','Karir_model','Master_model','Slip_gaji_model', 'Login_model'));
  ...
  public function karir($param=NULL,$id=NULL){
	...
	//run command periode gaji
	$command = "/var/www/html/gajian/assets/runner.sh";
	exec($command." 2>&1", $output);
    //exec('set HOME=/tmp; unoconv -f ods -o '.$file_name.'.ods '.$filename.' 2>&1', $output);
            print_r($output);
	
        /*if(!empty($getID)) #by yusuf 30 Agustus 2017
          redirect(site_url('karyawan/karir/detail/'.$getID));
        else//*/
          redirect(site_url('karyawan/karir/detail/'.$post['id_karyawan']));
      }
    }
    elseif($param=='ajax'){
      $post = $this->input->post(NULL, TRUE);
    ...
    public function changeportal(){
        ...
        if($status === "Aktif"){
            ...
            $newstatus = "1";
            if($this->Login_model->cekUser($idKaryawan)){ //jika ga ada di tabel users, buat baru uname dan passwd
                //true
            } else {
                //false
                echo '
                <div class"modal-content">
                            <form id="myModalForm" action="'.site_url('karyawan/newportal/'.$idKaryawan).'" role="form" class="form-horizontal" method="post">
                                <div class="modal-header">
                                    <h4 class="modal-title" id="myModalLabel">Tambah User Portal Karyawan</h4>
                                </div>
                                <div class="modal-body">
                                    <input type="hidden" name="id_data" id="id_data" value="'.$idKaryawan.'">
                                    <input type="text" name="username" id="id_data">Username</input>
                                    <input type="password" class="form-control" placeholder="Password" name="password1">
                                    <input type="password" class="form-control" placeholder="Password" name="password2">
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-default" data-dismiss="modal">Batal</button>
                                    <button type="submit" class="btn btn-primary" id="myModalSubmit">Buat</button>
                                </div>
                            </form>
                        </div>
                ';
        ...
        //meng-update status keaktifan portal di table user
        $where = array(
        ...
        $this->Karyawan_model->changeStatusPortal($where, $data);
