<?php
	...
	function get_template($view){
		$_this =& get_instance();
		return $_this->load->view('template/'.$view);
	}

	function get_segment($value){
		$_this =& get_instance();
		return $_this->uri->segment($value);
	}

	function is_selected($value,$data){
	...
	function aktif_page($page,$segment,$class){
		$_this =& get_instance();
		if($page == $_this->uri->segment($segment)){
			return $class;
		}
		return $_this->uri->segment($segment);
	}

	function get_data_db($model='',$id='',$key='',$single=FALSE){
		$_this =& get_instance();

		$get_model=$model.'_model';
		$_this->load->model($get_model);
		$none='<i> -None- </i>';

		if(!empty($id)){
			$data = $_this->$get_model->get($id);
			if(count($data)>0){
				if(!empty($key))
					return $data->$key;
				else
			...
			if($single==FALSE)
				return $_this->$get_model->get();
			else 
				return $none;
		}
	}
	function get_data_db_by($model, $where = NULL, $limit = NULL, $offset = NULL, $single = FALSE, $select = NULL){
		$_this =& get_instance();

		$get_model=$model.'_model';
		$_this->load->model($get_model);

		return $_this->$get_model->get_by($where, $limit, $offset, $single, $select);
	}

	

	function get_provinsi($value=NULL,$type=NULL,$nama='provinsi'){
		$options = array(
			0	=> 'Pilih Provinsi',
	        11	=> 'Aceh',
	        12	=> 'Sumatera Utara',
		...
		if($type=='form')
			return form_dropdown($nama, $options, $value, 'id="provinsi" class="form-control" required');
		else{
			if(!empty($value))
	...
	function get_kota($id='')
	{
		...
		$kota = json_decode($json_kota, true);
		foreach ($kota as $key => $value) {
		   if($value['id']==$id) return strtolower($value['name']);
		}
	}
	...
	function get_kecamatan($id='')
	{
		...
		$kecamatan = json_decode($json_kecamatan, true);
	...
	{
		$json_bank = file_get_contents("assets/json/bank.json");
		$bank = json_decode($json_bank, true);
		foreach ($bank as $key => $value) {
		   if($value['code']==$id) return $value['name'];
		}
	}	

	function get_JK($value){
		$options = array(
			''	=> '-',
	        'L'	=> 'Pria',
	        'P'	=> 'Wanita',
	...
	function get_data_option($value='3',$data='',$type='',$nama='option_data',$attribute=''){

		if($data=='pendapatan'){
			$options = array(
				'Jumlah Tetap'	=> 'Jumlah Tetap',
		        'Tergantung Kehadiran'	=> 'Tergantung Kehadiran',
		        '3'	=> 'Manual',
			);
		}
		elseif($data=='potongan'){
			$options = array(
		        '3'	=> 'Manual',
			);
		}
		...
		if($type=='form')
			return form_dropdown($nama, $options, $value, 'id="optionData" class="form-control" '.$attribute);
		else
			return $options[$value];
	}
	...
	function tombol_aktivasi($stat,$link){
		if($stat==0)
			$imgStat="<a href='$link' class='btn btn-sm btn-danger'>Non-Aktif</a>";
		elseif($stat==1)
			$imgStat="<a href='$link' class='btn btn-sm  btn-success'>Aktif</a>";
		
		$status="<span data-toggle='tooltip' data-placement='bottom' role='presentation' title='Ubah Status Keaktifan'>$imgStat</span>";
		
	return $status;
	}

	function get_data_periode($value='',$tipe='',$periode='')
	{
		$options=array(
			'Bulanan' => '1 Bulanan',
			'Mingguan' => '1 Mingguan',
		...
		if($periode=='Tetap'){
			if($tipe=='form')
				return form_dropdown('jangka', $options, $value, 'id="rentangWaktuPenggajian" class="form-control" ');
			else
				return $options[$value];
		}
		else{
	...
	function get_data($id=''){
		return get_data_db('Data',$id,'nama_data',TRUE);	
	}
	
	function get_karir_status($id=''){	
		return get_data_db('Master',$id,'nama_master',TRUE);
	}

	
	function get_data_param($param,$id){
		$_this =& get_instance();
		$data_id=array();
		$data_use=$_this->Slip_gaji_model->get_data_detail($id,$param);
		foreach($data_use as $key => $item) {
		        $data_id[] = $item->data_ID;         
		}
		$_this->db->where(array('perusahaan_ID' => get_user_info('perusahaan_ID'), 'kategori_data' => $param));
		if(!empty($data_id))
			$_this->db->where_not_in('data_ID',$data_id);
		
		$data = $_this->db->get('data_perusahaan')->result();
		

		$select='<select name="id_'.$param.'" id="option_'.$param.'" class="form-control">';
		
	
		foreach ($data as $key => $value) {
			
				 $select.= ' <option value="'.$value->data_ID.'">'.$value->nama_data.' ('.$value->option_data.')</option>';
		}
		$select.='</select>';
		return $select;
	...
	function get_gaji($value){
		$_this =& get_instance();

		$gaji=json_decode($value);
		$arrlength = count($gaji);
		$slip_gaji='';
		for($x=0; $x < $arrlength; $x++) {
			$get_gaji= $_this->Slip_gaji_model->get($gaji[$x])->master_gaji_nama;
			$slip_gaji.=', '.$get_gaji;
		}

		return trim(ltrim($slip_gaji, ','));

	}
	function menu_tgl($tanggal){
		$list='';
        ...
        $bln = $pecah[1];
        $thn = $pecah[0];
        $esok=date("Y-m-d", mktime(0,0,0,$bln,($tgl+6),$thn));
        //echo $new_tgl = date("l", mktime(0,0,0,$bln,($tgl+$i),$thn));
        $prev='
        <div class="col-xs-1 navigasiTanggal" onclick="document.location=\''.site_url('absensi/index/'.date("Y-m-d", mktime(0,0,0,$bln,($tgl),$thn))).'\'"><i class="fa fa-angle-double-left"></i></div>';
       	$list.=' 
       	<div class="col-xs-10">
            <div class="row tanggal visible-md visible-lg">
                <div>';
		for($i=1;$i<=9;$i++){
			if($i==5) $today='today'; else $today='';
			$hari_ini=date("d M Y", mktime(0,0,0,$bln,($tgl+$i),$thn));
			$nama_hari=nama_hari(date('Y-m-d',strtotime($hari_ini)));
			$link=date('Y-m-d',strtotime($hari_ini));
			$list.='<div class="col-xs-1 '.$today.'" onclick="document.location=\''.site_url('absensi/index/'.$link).'\'">'.$nama_hari.'<br>'.$hari_ini.'</div>';
		}

		$list.='
				</div>
            </div>
            <div class="row tanggal hidden-md hidden-lg">
                <div>
                	<div class="col-xs-12" onclick="document.location=\''.site_url('absensi/index/'.$esok).'\'">'.nama_hari($esok).', '.tgl_indo($esok).'</div>
                </div>
            </div>
        </div>';
        $next='	
        <div class="col-xs-1 navigasiTanggal" onclick="document.location=\''.site_url('absensi/index/'.date("Y-m-d", mktime(0,0,0,$bln,($tgl+10),$thn))).'\'"><i class="fa fa-angle-double-right"></i></div>';
